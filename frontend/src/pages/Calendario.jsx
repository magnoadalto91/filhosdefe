...
const SecLabel = ({ Icon, text }) => (
  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:10 }}>
    <Icon size={14} style={{ color:'#c8972b', flexShrink:0 }}/>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:20, fontFamily:"'Poppins',sans-serif" }}>
          ...
          {rotinas.length > 0 && (
            <div>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                {rotinas.map((r, i) => (
                  <div key={r.id} style={{ display:'flex', alignItems:'flex-start', gap:10, padding:'8px 12px', borderRadius:6, backgroundColor:'#f8f5f0', border:'1px solid #e5e0d8' }}>
                    <span style={{ fontSize:11, fontWeight:700, color:'#c8972b', minWidth:20, fontFamily:'monospace', marginTop:2 }}>{String(i+1).padStart(2,'0')}.</span>
                    <div>
...
function DayCell({ day, hasGira, isToday, onClick }) {
  ...
  return (
    <button
      ...
      style={{ width:'100%', aspectRatio:'1/1', borderRadius:4, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:2, backgroundColor:bg, border, color, cursor:hasGira?'pointer':'default', transition:'background-color 0.15s', fontFamily:"'Poppins',sans-serif" }}
    >
...
function GiraListCard({ gira, onClick }) {
  ...
  return (
    <button
      ...
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      style={{ width:'100%', textAlign:'left', display:'flex', alignItems:'center', gap:12, padding:'12px 14px', borderRadius:8, backgroundColor:'#ffffff', border:'1px solid #e5e0d8', boxShadow:hovered?'0 8px 28px rgba(0,0,0,0.12)':'0 2px 8px rgba(0,0,0,0.06)', cursor:'pointer', transition:'box-shadow 0.2s', fontFamily:"'Poppins',sans-serif" }}
    >
      <div style={{ flexShrink:0, width:44, height:44, borderRadius:6, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', backgroundColor:'#c8972b' }}>
        <span style={{ fontSize:16, fontWeight:700, lineHeight:1, color:'#ffffff' }}>{date.getDate()}</span>
        <span style={{ fontSize:10, lineHeight:1.2, color:'rgba(255,255,255,0.85)', textTransform:'uppercase' }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        ...
        {gira.descricao && (
          <div style={{ fontSize:12, color:'#6b7280', marginTop:2, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{gira.descricao}</div>
        )}
      </div>
    </button>
  )
...
export default function Calendario() {
  ...
  const navBtn = {
    ...
    borderRadius:4, color:'#c8972b', display:'flex', alignItems:'center', justifyContent:'center', transition:'background-color 0.15s',
  }

  return (
    <div style={{ padding:16, backgroundColor:'#ffffff', minHeight:'100%', fontFamily:"'Poppins',sans-serif" }}>
      ...
      <GiraDetailModal giraId={selectedGiraId} onClose={() => setSelectedGiraId(null)}/>
    </div>
  )
}
