...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.allSettled([
      api.get('/musicas'), api.get('/ervas'), api.get('/entidades'),
      api.get('/giras'),   api.get('/rotinas'),
    ]).then(([m, e, en, g, r]) => {
      ...
      setStats({ musicas:n(m), ervas:n(e), entidades:n(en), giras:n(g), rotinas:n(r) })
      ...
      <div style={{ marginBottom:28 }}>
        ...
        {loading ? <LoadingSpinner /> : (
          <div className="dash-stats" style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:16 }}>
            <style>{`@media(min-width:768px){.dash-stats{grid-template-columns:repeat(3,1fr)!important;}}@media(min-width:1280px){.dash-stats{grid-template-columns:repeat(5,1fr)!important;}}`}</style>
