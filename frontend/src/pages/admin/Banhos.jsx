...
import { Droplets, Search, Pencil, Trash2, AlertCircle, ImageIcon } from 'lucide-react'
...
const emptyForm = { nome:'', descricao:'', ingredientes:'' }

function BanhoFormWrapper({ form, setForm, error, preview, setPreview, fileRef }) {
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom:16 }}>
        ...
        <input style={S.input} value={form.nome} onChange={e=>setForm(f=>({...f,nome:e.target.value}))} placeholder="Nome do banho" onFocus={focus} onBlur={blur}/>
      </div>
      <div style={{ marginBottom:16 }}>
        <label style={S.label}>Descrição</label>
        <textarea style={{...S.input,resize:'vertical',minHeight:80}} value={form.descricao} onChange={e=>setForm(f=>({...f,descricao:e.target.value}))} placeholder="Para qual finalidade, ocasião, significado..." onFocus={focus} onBlur={blur}/>
      </div>
      <div style={{ marginBottom:16 }}>
        <label style={S.label}>Ingredientes</label>
        <textarea style={{...S.input,resize:'vertical',minHeight:120}} value={form.ingredientes} onChange={e=>setForm(f=>({...f,ingredientes:e.target.value}))} placeholder="Liste os ingredientes do banho..." onFocus={focus} onBlur={blur}/>
      </div>
    </div>
  )
}

export default function AdminBanhos() {
  ...
  const load = async () => {
    setLoading(true)
    try { const r = await api.get('/banhos'); setBanhos(Array.isArray(r.data)?r.data:r.data.banhos||[]) }
    catch { setBanhos([]) } finally { setLoading(false) }
  }
  useEffect(()=>{load()},[])
  ...
  const openEdit = b => {
    setEditTarget(b)
    setForm({ nome:b.nome||'', descricao:b.descricao||'', ingredientes:b.ingredientes||'' })
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        await api.put(`/banhos/${editTarget.id}`, fd)
      } else {
        await api.post('/banhos', fd)
      }
  ...
  const handleDelete = async () => {
    ...
    try { await api.delete(`/banhos/${deleteTarget.id}`); load() } catch{}
  }
  ...
  const handleBulkDelete = async () => {
    try { await Promise.all([...selectedIds].map(id => api.delete(`/banhos/${id}`))); setSelectedIds(new Set()); load() } catch{}
  }

  const filtered = banhos.filter(b => b.nome?.toLowerCase().includes(search.toLowerCase()))
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, marginBottom:24 }}>
        <div>
          <h1 style={{ margin:'0 0 4px', fontSize:22, fontWeight:800, color:'#2c2c3e' }}>Banhos</h1>
          <p style={{ margin:0, fontSize:13, color:'#6b7280' }}>{banhos.length} banho(s) cadastrado(s)</p>
        </div>
      ...
      <div style={{ position:'relative', marginBottom:16 }}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar banho..."
      ...
      {loading ? <LoadingSpinner/> : filtered.length===0 ? (
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'64px 0', gap:12 }}>
          <Droplets size={44} color="#e5e0d8"/>
          <p style={{ margin:0, fontSize:14, color:'#9ca3af' }}>Nenhum banho encontrado.</p>
        </div>
      ) : (
        <div className="banhos-grid" style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:16 }}>
          <style>{`@media(min-width:640px){.banhos-grid{grid-template-columns:repeat(3,1fr)!important;}}@media(min-width:1024px){.banhos-grid{grid-template-columns:repeat(4,1fr)!important;}}`}</style>
          {filtered.map(b => (
            <div key={b.id} style={{ backgroundColor:'#fff', borderRadius:10, border: selectedIds.has(b.id)?'2px solid #c8972b':'1px solid #e5e0d8', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.05)', position:'relative' }}>
              <input type="checkbox" checked={selectedIds.has(b.id)} onChange={()=>toggleSelect(b.id)} style={{ position:'absolute', top:8, left:8, zIndex:10, width:18, height:18, cursor:'pointer', accentColor:'#c8972b' }}/>
              <div style={{ aspectRatio:'4/3', backgroundColor:'#f8f5f0', position:'relative', overflow:'hidden' }}>
                {b.fotoUrl
                  ...
                  : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center' }}><Droplets size={32} color="#e5e0d8"/></div>}
              </div>
      ...
      <Modal isOpen={modalOpen} onClose={()=>setModalOpen(false)} title={editTarget?'Editar Banho':'Novo Banho'}
        footer={<>
      ...
      <ConfirmModal isOpen={!!deleteTarget} onClose={()=>setDeleteTarget(null)} onConfirm={handleDelete}
        title="Excluir Banho" message={`Excluir "${deleteTarget?.nome}"? Esta ação não pode ser desfeita.`}/>
      <ConfirmModal isOpen={showBulkConfirm} onClose={()=>setShowBulkConfirm(false)} onConfirm={handleBulkDelete}
        title="Excluir Banhos" message={`Excluir ${selectedIds.size} banho(s) selecionado(s)? Esta ação não pode ser desfeita.`}/>
    </div>
  )
}
