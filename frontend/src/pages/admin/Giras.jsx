...
function GiraForm({ form, setForm, error, assoc, setAssoc, allEntidades, allMusicas, allRotinas }) {
  ...
  return (
    <div>
      ...
      <MultiSelect
        label="Orixás / Entidades" Icon={Users}
        allItems={allEntidades} selectedIds={assoc.entidades}
        onChange={v=>setAssoc(a=>({...a,entidades:v}))} nameKey="nome"
      />
      <MultiSelect
        label="Músicas / Pontos" Icon={Music}
        allItems={allMusicas} selectedIds={assoc.musicas}
        onChange={v=>setAssoc(a=>({...a,musicas:v}))} nameKey="titulo"
      />
      <MultiSelect
        label="Rotinas" Icon={ListChecks}
        allItems={allRotinas} selectedIds={assoc.rotinas}
        onChange={v=>setAssoc(a=>({...a,rotinas:v}))} nameKey="titulo"
      />
    </div>
  )
}
...
/* ── GiraCard ─────────────────────────────────── */
function GiraCard({ gira, isPast, onEdit, onDelete }) {
...
export default function AdminGiras() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editTarget) {
        await api.put(`/giras/${editTarget.id}`, payload)
      } else {
        await api.post('/giras', payload)
      }
        ...
        <>
          {upcoming.length>0 && (
            <section style={{ marginBottom:28 }}>
              ...
              {[...upcoming].reverse().map(g=><GiraCard key={g.id} gira={g} isPast={false} onEdit={openEdit} onDelete={setDeleteTarget}/>)}
            </section>
          )}
          {past.length>0 && (
            <section>
              ...
              {past.map(g=><GiraCard key={g.id} gira={g} isPast={true} onEdit={openEdit} onDelete={setDeleteTarget}/>)}
            </section>
          )}
        </>
      )}
